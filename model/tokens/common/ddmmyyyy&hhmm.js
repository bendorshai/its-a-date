...
exports.tokens = [
    {
        ...
        affectsGenerator: function (match, settings) {
            ...
            // By default, if no day was found, its-a-date returns the first day of the month
            var day = 1;
            var month = match[this.variables.month];
    ...
    },
    {
        example: '1990.1.11',
        category: 'ddmmyyyy & hhmm',
        regex: /(?:^|\b)(\d{4})[\u2027\/\\.年-]{1,2}(\d{1,2})[\u2027\/\\.月日-]{1,2}(\d{1,2})[月日]?(?:$|\b)/,
        affectsGenerator: function (match, settings) {
